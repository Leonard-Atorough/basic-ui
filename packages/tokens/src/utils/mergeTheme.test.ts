...
const baseTheme: ThemeConfig = {
  color: {
    "color-background-secondary": "hsl(210 20% 97%)",
    "color-primary": "hsl(119 43% 52%)",
    "color-text-primary": "hsl(210 10% 18%)",
  },
...
describe("mergeTheme", () => {
  it.each<[string, Partial<ThemeConfig>]>([
    ["color", { color: { "color-primary": "#FF0000" } }],
  ...
  it("preserves base values across multiple categories", () => {
    const result = mergeTheme(baseTheme, { color: { "color-primary": "#FF0000" } });
  ...
  it("merges multiple categories simultaneously", () => {
    ...
    const result = mergeTheme(baseTheme, override);
    expect(result.color?.["color-primary"]).toBe("#0066FF");
